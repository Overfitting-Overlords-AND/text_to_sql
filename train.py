...
ds = dataset.TextToSqlData()
dl = torch.utils.data.DataLoader(ds, batch_size=constants.BATCH_SIZE, shuffle=True, collate_fn=ds.collate)

# Generate random sample data
# tgt_data = torch.randint(1, tgt_vocab_size, (64, max_seq_length))  # (batch_size, seq_length)

transformer = Transformer(constants.VOCAB_SIZE, constants.VOCAB_SIZE, constants.D_MODEL, constants.NUM_HEADS, constants.NUM_LAYERS, constants.D_FF, constants.MAX_SEQ_LENGTH, constants.DROPOUT).to(device)
...
for epoch in range(start_epoch, constants.NUM_OF_EPOCHS):
  ...
  for tgt_data in tqdm.tqdm(dl, desc=f"Epoch {epoch+1}/{constants.NUM_OF_EPOCHS}", unit="batch"):
    optimizer.zero_grad()
    output = transformer(tgt_data[0].to(device), tgt_data[1][:, :-1].to(device))
    loss = criterion(output.to(device).contiguous().view(-1, constants.VOCAB_SIZE), tgt_data[1][:, 1:].to(device).contiguous().view(-1))
    loss.backward()
    optimizer.step()
